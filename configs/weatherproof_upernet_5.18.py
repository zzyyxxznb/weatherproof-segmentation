...
pretrained = 'path/to/your/weight/upernet_internimage_h_896_160k_ade20k.pth'
model = dict(
    backbone=dict(
        _delete_=True,
        type='InternImage',
    ...
    decode_head=dict(num_classes=10, in_channels=[320, 640, 1280, 2560]),
    auxiliary_head=dict(num_classes=10, in_channels=1280),
...
data = dict(samples_per_gpu=1,
            ...
            test=dict(img_dir='images/scenes', ann_dir='annotations/val',pipeline=test_pipeline))
runner = dict(type='IterBasedRunner')
optimizer_config = dict(_delete_=True, grad_clip=dict(max_norm=0.1, norm_type=2))
checkpoint_config = dict(by_epoch=False, interval=500, max_keep_ckpts=1)
evaluation = dict(interval=160000, metric='mIoU', save_best='mIoU')
